...
def create_property(request):
    ...
    if form.is_valid():
        ...
        print('error', form.errors, form.non_fiels_errors)
...
def book_property(request, pk):
    try:
        ...
        if not all([start_date, end_date, total_price, guests]):
            return Response({'error': 'Missing required fields'}, status=status.HTTP_400_BAD_REQUEST)

        # Convert dates from strings to datetime.date
        ...
        number_of_nights = (end_date_obj - start_date_obj).days
